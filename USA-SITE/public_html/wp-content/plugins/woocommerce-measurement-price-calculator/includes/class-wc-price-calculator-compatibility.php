...
class WC_Price_Calculator_Compatibility {
	...
	public function google_product_feed_pricing_rules_price_adjustment( $feed_item ) {
		...
		if ( $product ) {
			...
			// quantity calculator with per unit pricing
				...
				// all other product types
					...
					if ( $measurement && $measurement_value ) {
						...
						$price         = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'price' )         / $measurement_value;
						$regular_price = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'regular_price' ) / $measurement_value;
						$sale_price    = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'sale_price' )    / $measurement_value;
					}
				}
			}
			...
			if ( ! empty( $price ) ) {
				$feed_item->price_inc_tax         = Framework\SV_WC_Product_Compatibility::wc_get_price_excluding_tax( $product, 1, $price );
				$feed_item->price_ex_tax          = Framework\SV_WC_Product_Compatibility::wc_get_price_including_tax( $product, 1, $price );
			}

			if ( ! empty( $regular_price ) ) {
				$feed_item->regular_price_ex_tax  = Framework\SV_WC_Product_Compatibility::wc_get_price_excluding_tax( $product, 1, $regular_price );
				$feed_item->regular_price_inc_tax = Framework\SV_WC_Product_Compatibility::wc_get_price_including_tax( $product, 1, $regular_price );
			}

			if ( ! empty( $sale_price ) ) {
				$feed_item->sale_price_ex_tax     = Framework\SV_WC_Product_Compatibility::wc_get_price_excluding_tax( $product, 1, $sale_price );
				$feed_item->sale_price_inc_tax    = Framework\SV_WC_Product_Compatibility::wc_get_price_including_tax( $product, 1, $sale_price );
			}
		}
