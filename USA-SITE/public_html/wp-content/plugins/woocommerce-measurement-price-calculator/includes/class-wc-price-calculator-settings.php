...
class WC_Price_Calculator_Settings {
	...
	public function __construct( $product = null ) {
		...
		if ( $product instanceof \WC_Product ) {
			...
			$settings = Framework\SV_WC_Product_Compatibility::get_meta( $product, '_wc_price_calculator', true );
		}
	...
	public function get_pricing_rules( $to_unit = null ) {
		...
		if ( $this->product && $this->is_pricing_calculator_enabled() ) {
			...
			if ( null === $this->pricing_rules ) {
				...
				$rules = Framework\SV_WC_Product_Compatibility::get_meta( $this->product, '_wc_price_calculator_pricing_rules', true );
