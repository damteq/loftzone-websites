	...
	private function get_total_stock( $product ) {
		...
		if ( count( $children ) > 0 ) {
			...
			foreach ( $children as $child_id ) {
				...
				if ( $child && 'yes' === Framework\SV_WC_Product_Compatibility::get_meta( $child, '_manage_stock', true ) ) {

					$stock        = Framework\SV_WC_Product_Compatibility::get_meta( $child, '_stock', true );
