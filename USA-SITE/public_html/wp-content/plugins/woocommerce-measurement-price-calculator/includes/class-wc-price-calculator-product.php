...
class WC_Price_Calculator_Product {
	...
	public static function get_dimension_measurement( $product, $measurements ) {
		 ...
		 */
		$measurement_value = apply_filters( 'wc_measurement_price_calculator_measurement_dimension', Framework\SV_WC_Product_Compatibility::get_prop( $product, $measurement_name, 'view' ), $product, $measurement );
	...
	public static function get_area_measurement( $product ) {
		...
		$measurement = null;
		$length      = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'length', 'view' );
		$width       = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'width', 'view' );
		...
		$area = Framework\SV_WC_Product_Compatibility::get_meta( $product, '_area', true );

		// fallback to parent meta for variations if not set
		if ( ! $area && $product->is_type( 'variation' ) ) {
			$area = Framework\SV_WC_Product_Compatibility::get_meta( Framework\SV_WC_Product_Compatibility::get_parent( $product ), '_area' );
		}
	...
	public static function get_surface_area_measurement( $product ) {
		...
		$length      = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'length', 'view' );
		$width       = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'width', 'view' );
		$height      = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'height', 'view' );
	...
	public static function get_volume_measurement( $product ) {
		...
		if ( ! empty( $area ) && is_numeric( $height ) ) {
			...
			$dimension_measurement = new \WC_Price_Calculator_Measurement( $dimension_unit, Framework\SV_WC_Product_Compatibility::get_prop( $product, 'height', 'view' ) );
		...
		$volume = Framework\SV_WC_Product_Compatibility::get_meta( $product, '_volume', true );

		// fallback to parent meta for variations if not set
		if ( ! $volume && $product->is_type( 'variation' ) ) {
			$volume = Framework\SV_WC_Product_Compatibility::get_meta( Framework\SV_WC_Product_Compatibility::get_parent( $product ), '_volume' );
		}
	...
	public static function get_weight_measurement( $product ) {
		...
		$weight = Framework\SV_WC_Product_Compatibility::get_prop( $product, 'weight', 'view' );
	...
	public static function calculate_price( $product, $measurement_needed_value, $measurement_needed_value_unit, $round = false ) {
		...
		if ( self::pricing_calculator_enabled( $parent ) ) {
			...
			$min_price = Framework\SV_WC_Product_Compatibility::get_meta( $product, '_wc_measurement_price_calculator_min_price', true );
	...
	public static function variable_product_sync( $product, $settings ) {
		...
		Framework\SV_WC_Product_Compatibility::set_props( $product, $product_new_prices );
		...
		foreach ( $product->get_children() as $variation_product_id ) {
			...
			$child_price         = Framework\SV_WC_Product_Compatibility::get_prop( $variation_product, 'price' );
			$child_regular_price = Framework\SV_WC_Product_Compatibility::get_prop( $variation_product, 'regular_price' );
			$child_sale_price    = Framework\SV_WC_Product_Compatibility::get_prop( $variation_product, 'sale_price' );
