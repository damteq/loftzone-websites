...
class WC_Price_Calculator_Product_Page {
	...
	public function price_per_unit_html( $price_html, $product ) {
		...
		if ( $product->is_type( 'variation' ) ) {
			$_product = Framework\SV_WC_Product_Compatibility::get_parent( $product );
		}
				...
				// other product types
					...
					if ( $measurement && '' !== $price_html && $measurement->get_value() ) {
						...
						$original_prices = array(
							'price'         => Framework\SV_WC_Product_Compatibility::get_prop( $product, 'price' ),
							'regular_price' => Framework\SV_WC_Product_Compatibility::get_prop( $product, 'regular_price' ),
							'sale_price'    => Framework\SV_WC_Product_Compatibility::get_prop( $product, 'sale_price' ),
						);
						...
						Framework\SV_WC_Product_Compatibility::set_props( $product, $new_prices );
						...
						Framework\SV_WC_Product_Compatibility::set_props( $product, $original_prices );
	...
	public function available_variation( $variation_data, $product, $variation ) {
		...
		$variation_data['price'] = Framework\SV_WC_Product_Compatibility::wc_get_price_to_display( $variation );
	...
	public function enqueue_frontend_scripts() {
		...
		if ( $product && \WC_Price_Calculator_Product::calculator_enabled( $product ) ) {
			...
			$wc_price_calculator_params['minimum_price'] = is_numeric( $min_price ) ? Framework\SV_WC_Product_Compatibility::wc_get_price_to_display( $product, $min_price ) : '';

			// information required for either pricing or quantity calculator to function
			$wc_price_calculator_params['product_price'] = $product->is_type( 'variable' ) ? '' : Framework\SV_WC_Product_Compatibility::wc_get_price_to_display( $product );
	...
	public function render_price_calculator() {
		...
		if ( \WC_Price_Calculator_Product::pricing_calculator_enabled( $product ) ) {
			...
			if ( $product->is_type( 'simple' ) ) {
				...
				$total_price = wc_price( $quantity_range['min_value'] * Framework\SV_WC_Product_Compatibility::wc_get_price_to_display( $product ), 2 );
