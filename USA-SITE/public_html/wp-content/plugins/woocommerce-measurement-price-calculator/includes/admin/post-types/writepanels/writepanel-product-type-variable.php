...
function wc_price_calculator_product_after_variable_attributes( $loop, $variation_data, $variation_post ) {
	...
	$parent_data = array(
		'area'   => $post ? Framework\SV_WC_Product_Compatibility::get_meta( $parent_product, '_area', true )   : null,
		'volume' => $post ? Framework\SV_WC_Product_Compatibility::get_meta( $parent_product, '_volume', true ) : null,
	);
	...
	if ( isset( $_POST['variable_sku'] ) ) {
		...
		for ( $i = 0; $i <= $max_loop; $i++ ) {
			...
			if ( empty( $variable_area[ $i ] ) ) {
				Framework\SV_WC_Product_Compatibility::delete_meta_data( $variation, '_area' );
			} else {
				Framework\SV_WC_Product_Compatibility::update_meta_data( $variation, '_area', $variable_area[ $i ] );
			}

			// Update volume post meta
			if ( empty( $variable_volume[ $i ] ) ) {
				Framework\SV_WC_Product_Compatibility::delete_meta_data( $variation, '_volume' );
			} else {
				Framework\SV_WC_Product_Compatibility::update_meta_data( $variation, '_volume', $variable_volume[ $i ] );
			}
		}
	}
}
